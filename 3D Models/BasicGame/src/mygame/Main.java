...
public class Main extends SimpleApplication {
    ...
    final private static float WATER_HEIGHT = 30;
    private WaterFilter[] fiters = new WaterFilter[3];
    final private Float[] maxHeight = {120f, 120f, 130f};
    
    JMonkeyClient client = null;
    BitmapText[] hudText;
    BitmapText hudTextR;
    ...
    public void simpleInitApp() {
        ...
        String text = "";
        for(int i = 0; i < 3; i++) {
            text += "Lake " + i + " - Depth: " + fiters[i].getWaterHeight() + "\n";
        }
        BitmapText hudText1 = new BitmapText(guiFont, false);
        hudText1.setSize(guiFont.getCharSet().getRenderedSize());      // font size
        hudText1.setColor(ColorRGBA.Blue);                             // font color
        hudText1.setText("Lake 1 - Depth: " + fiters[0].getWaterHeight());
        hudText1.setLocalTranslation(10, 100, 0); // position
        
        
        BitmapText hudText2 = new BitmapText(guiFont, false);
        hudText2.setSize(guiFont.getCharSet().getRenderedSize());      // font size
        hudText2.setColor(ColorRGBA.Brown);                             // font color
        hudText2.setText("Lake 2 - Depth: " + fiters[1].getWaterHeight());
        hudText2.setLocalTranslation(10, 80, 0); // position
        
        
        BitmapText hudText3 = new BitmapText(guiFont, false);
        hudText3.setSize(guiFont.getCharSet().getRenderedSize());      // font size
        hudText3.setColor(ColorRGBA.Red);                             // font color
        hudText3.setText("Lake 3 - Depth: " + fiters[2].getWaterHeight());
        hudText3.setLocalTranslation(10, 60, 0); // position
        hudText = new BitmapText[3];
        hudText[0] = hudText1;
        hudText[1] = hudText2;
        hudText[2] = hudText3;
        
        
        hudTextR = new BitmapText(guiFont, false);
        hudTextR.setSize(guiFont.getCharSet().getRenderedSize());      // font size
        hudTextR.setColor(ColorRGBA.Black);                             // font color
        hudTextR.setText("No response");
        hudTextR.setLocalTranslation(230, 100, 0); // position
        guiNode.attachChild(hudText1);
        guiNode.attachChild(hudText2);
        guiNode.attachChild(hudText3);
        guiNode.attachChild(hudTextR);
    }
    ...
    public void simpleUpdate(float tpf) {
        String text = "";
        for(int i = 0; i < 3; i++) {
            Double v = ThreadLocalRandom.current().nextDouble(-1.5, 1.6);
            if(fiters[i].getWaterHeight() > 100) {
                v = ThreadLocalRandom.current().nextDouble(-1.6, 1.5);
            }
            if(ThreadLocalRandom.current().nextDouble(0, 1) < 0.6) {
                if(fiters[i].getWaterHeight() + v < maxHeight[i] && fiters[i].getWaterHeight() + v > 0f) {
                    fiters[i].setWaterHeight(fiters[i].getWaterHeight() + v.floatValue());
                    
                    hudText[i].setText("Lake " + (i + 1) + " - Depth: " + fiters[i].getWaterHeight());
                    Request r = new Request("Sensor" + Integer.toString(i + 1), new Double(fiters[i].getWaterHeight()));
                    if(client != null) {
        ...
        if(client != null) {
            for(String k : client.getModelValue().keySet()) {
                text += k + " - " + client.getModelValue().get(k) + "\n";
            }
            if(client.getModelValue().keySet().size() > 0) {
                hudTextR.setText(text);
            }
        }
    }
}